...
/*   Updated: 2022/10/21 15:23:07 by mtellal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pick_pixel_img2d(t_frame *minimap, t_coor pos_mnmap, int i, int j)
{
	...
	addr = minimap->addr;
	h = (int)pos_mnmap.y * minimap->length + i * minimap->length;
	l = (int)pos_mnmap.x * (minimap->bpp / 8) + j * (minimap->bpp / 8);
	return (*(int *)(addr + h + l));
}
...
void	minimap(t_data *data, t_frame *minimap, t_frame *img3d)
{
	...
	minimap_init_img3d(img3d, &size_minimap_img3d, &pos_minimap_img3d);
	pos_player.x = minimap->triangle.milieu.x;
	pos_player.y = minimap->triangle.milieu.y;
	pos_minimap_img2d.x = pos_player.x - size_minimap_img3d / 2;
	pos_minimap_img2d.y = pos_player.y - size_minimap_img3d / 2;
	if (pos_minimap_img2d.x < 0)
		pos_minimap_img2d.x = 0;
	if (pos_minimap_img2d.y < 0)
		pos_minimap_img2d.y = 0;
	display_map(data, size_minimap_img3d, pos_minimap_img2d, pos_minimap_img3d);
}
