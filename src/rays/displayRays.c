...
void	displayCieling(t_frame *img, t_texture *texture, int posx, int posy, int l, int color)
{
		...
		double ratio_imgwall_strip_h, int posy_strip_imgwall)
{
	...
	// plafond
	//////////// PAS ASSEZ JUSTE UNE BARRE ET NON 2 (l = 2 et non 1)
	
	displayCieling(img, texture, posx, posy, l, color);
...
/* 			else if (color && posy_strip_imgwall == 0)
	...
	while (si < HEIGHT2)
	{
		put_pixel(img, si, posy, FLOORCOLOR);
		si++;
	}
...
///////////////////////////////////////////////////////////////////////////////////
	...
	double scale_y = (double)texture->wall.height / h;
	...
	int posywall = poslwall * texture->wall.width / GRID;
	...
	displayRay(img, posx, posy + (int)l * i, h, l, color, texture, posxwall, scale_y, posywall);
